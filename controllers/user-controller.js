...
exports.getassignedlawyer = async (req, res) => {
  ...
  if (!companyrequest) { return res.status(404).send({ error: 'No such Request' }) }
  if (!companyrequest.assigned) { return res.status(404).send({ error: 'lawyer is not assigned yet' }) }
