...
test("update Electronic Journal", async () => {
  ...
  const EJ = await functions.createElectronicJournal(
    "electronicJournalcompany",
    company.data.company_name_english
  );
  const newValue = uuid.v4();
  const response = await functions.updateElectronicJournal(
    EJ.data.data._id,
    newValue,
    company.data.company_name_english
  );
...
test("check the company comments", async () => {
  ...
  const commentcreated = await functions.createComment(
    "the comment is created succ",
    company.data._id,
    user1.data.data._id
  );
...
test("update comment", async () => {
  ...
  const response = await functions.updateComment(
    commentcreated.data.comment._id,
    "hey user"
  );
...
test("assign reviewer 2", async () => {
  ...
  const companyafterupdate = await functions.assignreviewer(
    company.data._id,
    reviewer.data.data._id
  );
...
test("get notifications", async () => {
  ...
  expect(response.data).toContainEqual(
    expect.objectContaining({ _id: notification.data._id })
  );
});
...
test("set notification viewed", async () => {
  ...
  const response = await functions.getUserNotifications(user.data.data._id);
  expect(response.data).toContainEqual(
    expect.objectContaining({ _id: notification.data._id, viewed: true })
  );
});
...
test("assign lawyer", async () => {
  ...
  const notifications = await functions.getUserNotifications(
    lawyer.data.data._id
  );
  expect(notifications.data).toContainEqual(
    expect.objectContaining({
      target_type: "company",
      target_id: company.data._id
    })
  );
});
...
test("get assigned lawyer for request", async () => {
  ...
  await functions.assignLawyerRequest(request.data.data._id, {
    lawyer_id: lawyer.data.data._id
  });
  const assignedLawyer = await functions.getAssignedLawyer(
    request.data.data._id
  );

  expect(lawyer.data.data).toEqual(
    expect.objectContaining(assignedLawyer.data)
  );
}),
...
test("list all Unassigned Applications", async () => {
  ...
  expect(all.data).not.toContainEqual(
    expect.objectContaining({ _id: company.data._id })
  );
});
...
test("Admin publish paid application", async () => {
  ...
  const publish = await functions.publishPaidApplication(
    company.data._id,
    admin.data.data._id
  );
